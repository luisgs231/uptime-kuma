...
describe("Managing accounts", () => {
    ...
    test("an empty password is allowed, and signs in", async () => {
        const login = require("../../server/auth").login;

        await users.createUser({ username: "alice",
            password: "" });
        assert.ok(await login("alice", ""), "an empty password must be accepted at login");
        assert.strictEqual(await login("alice", "something"), null, "and nothing else may sign in");
    });

    test("an account can be reset to an empty password", async () => {
        const login = require("../../server/auth").login;

        const id = await users.createUser({ username: "alice",
            password: "hunter2" });
        await users.setUserPassword(id, "");
