...
const { startImported, importAccountOrUndo } = require("../../server/import/importer");
...
describe("Starting what was imported", () => {
    const testDB = new TestDB(scratchDir("uptime-kuma-test-import-start"));
    let alice;
    let bob;
    ...
    /**
     * Add a monitor owned by somebody.
     * @param {number} userID Owner
     * @param {string} name Monitor name
     * @param {number} active Whether it is running
     * @returns {Promise<number>} The monitor id
     */
    async function monitor(userID, name, active) {
        const inserted = await R.knex("monitor").insert({ name,
            user_id: userID,
            active,
            type: "http",
            interval: 60 });
    ...
    test("every active monitor is started, and the paused ones are not", async () => {
        const running = await monitor(alice, "running", 1);
        const alsoRunning = await monitor(alice, "also running", 1);
        await monitor(alice, "paused", 0);

        const started = [];
        const count = await startImported(alice, (userID, monitorID) => started.push([ userID, monitorID ]));

        assert.strictEqual(count, 2);
        assert.deepStrictEqual(started.sort((a, b) => a[1] - b[1]),
            [ [ alice, running ], [ alice, alsoRunning ] ].sort((a, b) => a[1] - b[1]));
    });

    test("somebody else's monitors are left alone", async () => {
        await monitor(bob, "bob's", 1);
        const mine = await monitor(alice, "mine", 1);

        const started = [];
        await startImported(alice, (userID, monitorID) => started.push(monitorID));

        assert.deepStrictEqual(started, [ mine ], "an import must not restart another account's monitors");
    });

    test("one that refuses to start does not strand the rest", async () => {
        const first = await monitor(alice, "first", 1);
        const second = await monitor(alice, "second", 1);

        const started = [];
        const count = await startImported(alice, (userID, monitorID) => {
            if (monitorID === first) {
                throw new Error("nope");
            }
            started.push(monitorID);
        });

        assert.strictEqual(count, 1);
        assert.deepStrictEqual(started, [ second ]);
    });

    test("an import of nothing starts nothing", async () => {
        assert.strictEqual(await startImported(alice, () => {
            throw new Error("should not be called");
        }), 0);
    });
});

describe("The import actually asks for them to be started", () => {
    test("the socket handler calls startImported", () => {
        const fs = require("fs");
        const path = require("path");
        const handler = fs.readFileSync(
            path.resolve(__dirname, "../../server/socket-handlers/import-socket-handler.js"), "utf8");

        // The rows landing in the database is not the end of it: without this
        // call an import looks like it worked and nothing beats until the next
        // restart. That is exactly the bug this test exists for.
        assert.match(handler, /startImported\(/, "the handler must start what it imported");
        assert.match(handler, /importSocketHandler = \(socket, startMonitor\)/,
            "and it needs startMonitor passed in to do it");
    });

    test("server.js passes startMonitor to the handler", () => {
        const fs = require("fs");
        const path = require("path");
        const server = fs.readFileSync(path.resolve(__dirname, "../../server/server.js"), "utf8");
        assert.match(server, /importSocketHandler\(socket, startMonitor\)/);
    });
});
