...
const { importAccountOrUndo, startImported } = require("../import/importer");
const { createUser, deleteUser } = require("../user-management");

/**
 * Importing an account from another Uptime Kuma.
 * @param {Socket} socket Socket.io instance to add listeners on
 * @param {Function} startMonitor Starts one monitor, given (userID, monitorID)
 * @returns {void}
 */
module.exports.importSocketHandler = (socket, startMonitor) => {
    /**
    ...
    socket.on("importCreateUser", async (config, sourceUserID, options, callback) => {
        ...
        try {
            ...
            // Copying the rows is not enough - the running server has to be
            // told, or nothing beats until the next restart.
            const started = await startImported(newUserID, startMonitor);

            log.info("import", `Imported ${counts.monitor ?? 0} monitor(s) into ${username}, started ${started}`);
