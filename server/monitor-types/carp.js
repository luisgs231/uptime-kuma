...
const {
    CARP_STATUS_META,
    formatStatusPrefix,
    statusFromMessage: sharedStatusFromMessage,
...
function parseConfig(raw) {
    ...
    return {
        ...
        ...parsed,
        selector,
        expectedRole: normaliseRole(parsed.expectedRole),
...
class CarpMonitorType extends MonitorType {
    ...
    async check(monitor, heartbeat, _server) {
        ...
        if (!config.floatingIp) {
            this.applyStatus(heartbeat, VIP_MISSING, "No floating IP is configured", config);
            return;
        }
        ...
        let detail = message;
        if (apiAvailable && unreachable.length && status !== VIP_DOWN) {
        ...
        await maybeNotify(monitor, heartbeat, {
            ...context,
            previousStatus: statusFromMessage(context.previousMsg),
    ...
    applyStatus(heartbeat, status, message, config) {
        ...
        heartbeat.status = displayToHeartbeatStatus(display);
    ...
    async queryNode(ip, config) {
        ...
        try {
            ...
            const vips = parseVipStatus(response.data);
            const selector = config.vhid ? { vhid: config.vhid,
                address: config.floatingIp } : { address: config.floatingIp };
            let vip = findVip(vips, selector);
            if (!vip && config.vhid) {
                vip = findVip(vips, { vhid: config.vhid });
            }

            result.reachable = true;
            result.role = vip ? normaliseRole(vip.status) : null;
            if (!vip) {
