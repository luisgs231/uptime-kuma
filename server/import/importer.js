...
/**
 * Start what was just imported.
 *
 * Copying the rows is not enough: a monitor only beats once the running server
 * has been told about it. Without this the import looks like it worked and
 * nothing is ever checked until the next restart, when startMonitors() sweeps
 * up everything marked active.
 * @param {number} userID Account the monitors were imported into
 * @param {Function} start Starts one monitor, given (userID, monitorID)
 * @returns {Promise<number>} How many were started
 */
async function startImported(userID, start) {
    const rows = await R.knex("monitor")
    ...
    for (const row of rows) {
        try {
            await start(userID, row.id);
            started++;
        } catch (e) {
            // One monitor that will not start must not strand the rest.
            log.warn("import", `Imported monitor ${row.id} did not start: ${e.message}`);
        }
    }
    return started;
}
